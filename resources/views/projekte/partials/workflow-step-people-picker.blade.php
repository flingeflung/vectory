        ...
        <div class="space-y-1" x-ref="list">
            @foreach ($visibleMembers as $person)
                <label class="flex items-center gap-1.5 text-sm {{ $person->active ? 'text-gray-700' : 'text-gray-400' }}">
                    ...
                    @if ($person->email)
                        <span class="text-xs text-gray-400">{{ $person->email }}</span>
                    @endif
                    <x-absence-icon :person="$person" />@if ($primaryPersonId === $person->id)<span class="text-amber-500" title="{{ __('Erstansprechpartner') }}">&#9733;</span>@endif
                </label>
            @endforeach
        </div>
    @endif
