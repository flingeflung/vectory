@php
    ...
    $isWideField = function ($field) {
        if ($field->system) {
            return in_array($field->key, ['title', 'start_date', 'status', 'remarks', 'markets', 'project_people', 'project_connections', 'remarks_echo', 'change_log', 'date_progress', 'progress'], true);
        }
        ...
        <div class="{{ $isOverlay ? 'shrink-0 px-4 pt-3' : '' }}">
            <div class="mb-3 flex items-start justify-between gap-3">
                ...
                {{-- Ralf, 2026-09-20 (Stamm-ID): sichtbar, aber nicht von Hand
                     änderbar - kopierbar für die Suche. Der Button zur
                     Versionsübersicht steht bei der Version (Stammdaten). --}}
                @if ($project->stamm_id)
