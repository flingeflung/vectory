    ...
    <div class="max-h-80 space-y-2 overflow-y-auto">
        @forelse ($businessUnits as $unit)
            <div class="rounded-md border border-gray-200 p-2" x-data="{}">
                <form data-row-form x-data="{ dirty: false }" @input="dirty = window.formIsDirty($el)" method="POST" action="{{ route('admin.geschaeftsbereiche.update', $unit) }}" class="flex items-end gap-2">
                    @csrf
                    <div class="flex-1">
                        <label class="block text-xs text-gray-500">{{ __('Name') }}</label>
                ...
                <form method="POST" action="{{ route('admin.geschaeftsbereiche.destroy', $unit) }}" x-ref="deleteForm" class="hidden">
                    @csrf
                    @method('DELETE')
                ...
                <div class="mt-2 flex justify-end">
                    <button
                        ...
                        @click="window.deleteWithConfirm($refs.deleteForm, {
                            message: {{ \Illuminate\Support\Js::from($unit->people_count > 0 ? trans_choice('Wird bereits von :count Person verwendet.|Wird bereits von :count Personen verwendet.', $unit->people_count, ['count' => $unit->people_count]).' '.__('Ohne Umhängen wird die Geschäftsbereichszuweisung auf „– nicht zugewiesen –“ gesetzt.') : __('Diesen Geschäftsbereich wirklich endgültig löschen?')) }},
                            reassignOptions: @js($unit->people_count > 0 ? $businessUnits->where('id', '!=', $unit->id)->map(fn ($target) => ['value' => (string) $target->id, 'label' => $target->name])->values() : []),
                        })"
