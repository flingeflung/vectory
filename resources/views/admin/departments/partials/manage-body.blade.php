    ...
    <div class="max-h-80 space-y-2 overflow-y-auto">
        @forelse ($departments as $department)
            <div class="rounded-md border border-gray-200 p-2" x-data="{}">
                <form data-row-form x-data="{ dirty: false }" @input="dirty = window.formIsDirty($el)" method="POST" action="{{ route('admin.departments.update', $department) }}" class="flex items-end gap-2">
                    @csrf
                    <div class="flex-1">
                        <label class="block text-xs text-gray-500">{{ __('Name') }}</label>
