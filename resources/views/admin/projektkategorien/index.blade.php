        ...
        <div class="flex flex-1 min-h-0 flex-col rounded-lg border border-gray-200 bg-white">
            @if ($selectedCategory)
                <div
                    x-data="{}"
                    x-init="
                        window.adminPageIsDirty = () => window.__projektkategorienDirtyForms.size > 0;
                        window.addEventListener('beforeunload', (e) => { if (window.__projektkategorienDirtyForms.size > 0) { e.preventDefault(); e.returnValue = ''; } });
                    "
                ...
                >
                    ...
                    @forelse ($selectedCategory->subs as $sub)
                        ...
                        <div x-sort:item="{{ $sub->id }}" x-data="{ rowDirty: false }" class="rounded-md border border-gray-200 p-2">
                            <form method="POST" action="{{ route('admin.projektkategorien.arten.update', $sub) }}" class="flex items-center gap-2" @input="rowDirty = true; window.__projektkategorienDirtyForms.add($el)" @submit="rowDirty = false; window.__projektkategorienDirtyForms.delete($el)">
                                @csrf
