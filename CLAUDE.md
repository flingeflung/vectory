...
- **Filter lösen automatisch aus**: Pulldowns in einem Filterbereich lösen die Filterung sofort per `onchange="this.form.submit()"` aus, kein separater "Filtern"-Button nötig. Textfelder (z. B. Namenssuche) sind **Live-Suche**, kein normaler Seiten-Reload: `oninput="window.liveFilterSearch(this, '<result-container-id>')"` (global definiert in `resources/views/layouts/app.blade.php`) - kurzes Delay (400ms, NICHT 1-2s, das fühlt sich sonst träge an), tauscht nur den per ID angegebenen Ergebnis-Container per `fetch()` aus statt die ganze Seite neu zu laden. Wichtig: das Suchfeld selbst darf NICHT Teil des ausgetauschten Containers sein, sonst verliert es beim Tippen Fokus/Cursor-Position (das war zweimal ein konkreter Bug-Report von Ralf). Beispiele: `resources/views/admin/personen/index.blade.php` (`#personen-list`), `resources/views/illustrationen/index.blade.php` (`#illustrationen-list`). Gilt für serverseitige Filter-Formulare (GET), nicht zu verwechseln mit rein clientseitigen Alpine-Filtern (die schon immer sofort reagieren) oder dem Projektfilter (dort bewusst kein Auto-Submit, siehe eigene Ralf-Entscheidung).
...
- **Nie native Browser-Dialoge (`confirm()`, `alert()`)**: immer den gestylten globalen Dialog verwenden - `await window.confirmDialog({ title, message, confirmLabel, cancelLabel })` (oder als einfacher String für die Standard-Formulierung "Ungespeicherte Änderungen") für Ja/Nein-Bestätigungen, `await window.notifyDialog(message)` für reine Hinweise mit nur einem OK-Button. Komponente: `resources/views/components/confirm-dialog.blade.php` (bereits global in `layouts/app.blade.php` eingebunden). Bei einem `<form>`-Submit, der bestätigt werden muss: `x-data="{ async confirmAndSubmit(e) { if (await window.confirmDialog({...})) { e.target.submit(); } } }"` + `@submit.prevent="confirmAndSubmit($event)"` statt `onsubmit="return confirm(...)"`. Ralfs Begründung: native Dialoge sehen unprofessionell aus und brechen den Look des Tools.
- **Löschen mit möglicher Personen-Zuordnung → Konsequenz explizit nennen**: wenn ein Löschen-Vorgang Personen betrifft, die dem gelöschten Datensatz zugeordnet sind (Firma/Abteilung/Geschäftsbereich/Rolle), UND die Übernahme in einen anderen Datensatz optional ist (nicht `required`), muss der erklärende Text die Konsequenz beim Nicht-Auswählen explizit benennen (z. B. "Wird die Firma gelöscht, ohne die betroffene(n) Person(en) einer anderen Firma zuzuweisen, wird ihre Firmenzuweisung auf „– nicht zugewiesen –“ gesetzt."), nicht nur neutral "N Personen zugeordnet" o. ä. Löschen-Aktionen mit Personenbezug sind außerdem standardmäßig eingeklappt (nur ein kleiner "Löschen"-Button), erst ein Klick öffnet die Erklärung + Übernahme-Auswahl + "Abbrechen"/"Endgültig löschen" - kein Dauerzustand mit immer sichtbarem Pulldown. Beispiele: `resources/views/admin/companies/partials/manage-body.blade.php`, `admin/departments/`, `admin/business-units/`, `admin/legacy-roles/`.
